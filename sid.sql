...
--
-- Host: 127.0.0.1
-- Generation Time: 09-Maio-2019 às 20:35
...
--
-- Estrutura da tabela `sid_permissoes`
--

CREATE TABLE `sid_permissoes` (
  `id_permissao` int(11) NOT NULL,
  `descricao` text NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Extraindo dados da tabela `sid_permissoes`
--

INSERT INTO `sid_permissoes` (`id_permissao`, `descricao`) VALUES
(0, 'inativo'),
(1, 'investigador'),
(2, 'migrador'),
(3, 'auditor'),
...
--
-- Acionadores `sid_permissoes`
--
DELIMITER $$
CREATE TRIGGER `sid_permissoes_delete` AFTER DELETE ON `sid_permissoes` FOR EACH ROW BEGIN
	INSERT INTO SID.SID_LOG (momento_criacao,mensagem,operacao, username) values (SYSDATE(), CONCAT('id_permissao: ', OLD.id_permissao, ' descricao: ' ,OLD.descricao), 'delete',CURRENT_USER);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `sid_permissoes_insert` AFTER INSERT ON `sid_permissoes` FOR EACH ROW BEGIN
	INSERT INTO SID.SID_LOG (momento_criacao,mensagem,operacao, username) values (SYSDATE(), CONCAT('id_permissao: ', NEW.id_permissao, ' descricao: ' ,NEW.descricao), 'insert', CURRENT_USER);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `sid_permissoes_update` AFTER UPDATE ON `sid_permissoes` FOR EACH ROW BEGIN  
	IF  OLD.id_permissao <> NEW.id_permissao THEN 
		SET @tipo = 'id_permissao';
        SET @antigo = OLD.id_permissao;
        SET @novo = NEW.id_permissao;
	ELSEIF OLD.descricao <> NEW.descricao  THEN 
		SET @tipo = 'descricao';
        SET @antigo = OLD.descricao;
        SET @novo = NEW.descricao;
	END IF;    
	INSERT INTO SID.SID_LOG (momento_criacao,mensagem,operacao, username) values (SYSDATE(), concat('Campo: ' , tipo, 'alterado de ', antigo, ' para ', novo), 'update', CURRENT_USER);
END
$$
...
CREATE TRIGGER `sid_user_delete` AFTER DELETE ON `sid_user` FOR EACH ROW BEGIN
	INSERT INTO SID.SID_LOG (momento_criacao,mensagem,operacao, username) values (SYSDATE(), CONCAT('id_user : ', OLD.id_user  , ' user_name : ' , OLD.user_name, ' email : ' ,OLD.email), 'delete', CURRENT_USER);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `sid_user_insert` AFTER INSERT ON `sid_user` FOR EACH ROW BEGIN
	INSERT INTO SID.SID_LOG (momento_criacao,mensagem,operacao, username) values (SYSDATE(), CONCAT('id_user : ', NEW.id_user  , ' user_name : ' ,NEW.user_name, ' email : ' ,NEW.email), 'insert', CURRENT_USER);
END
$$
DELIMITER ;
DELIMITER $$
...
CREATE TRIGGER `sid_user_permissoes_delete` AFTER DELETE ON `sid_user_permissoes` FOR EACH ROW BEGIN
	INSERT INTO SID.SID_LOG (momento_criacao,mensagem,operacao, username) values (SYSDATE(), CONCAT('id_user: ', OLD.id_user, ' id_permissao: ' ,OLD.id_permissao), 'delete', CURRENT_USER);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `sid_user_permissoes_insert` AFTER INSERT ON `sid_user_permissoes` FOR EACH ROW BEGIN
	INSERT INTO SID.SID_LOG (momento_criacao,mensagem,operacao, username) values (SYSDATE(), CONCAT('id_user: ', NEW.id_user, ' id_permissao: ' ,NEW.id_permissao), 'insert', CURRENT_USER);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `sid_user_permissoes_update` AFTER UPDATE ON `sid_user_permissoes` FOR EACH ROW BEGIN  
	IF  OLD.id_permissao <> NEW.id_permissao THEN 
        ...
        SET @antigo = OLD.id_permissao;
        SET @novo = NEW.id_permissao;
	ELSEIF OLD.id_user <> NEW.id_user  THEN 
		SET @tipo = 'id_user';
        SET @antigo = OLD.id_user;
        SET @novo = NEW.id_user;
	END IF;    
...
--
-- Indexes for table `sid_permissoes`
--
ALTER TABLE `sid_permissoes`
...
--
-- Indexes for table `sid_user_permissoes`
--
ALTER TABLE `sid_user_permissoes`
...
--
-- Limitadores para a tabela `sid_user_permissoes`
--
ALTER TABLE `sid_user_permissoes`
  ADD CONSTRAINT `id_permissao` FOREIGN KEY (`id_permissao`) REFERENCES `sid_permissoes` (`id_permissao`) ON UPDATE CASCADE,
  ADD CONSTRAINT `id_user` FOREIGN KEY (`id_user`) REFERENCES `sid_user` (`id_user`) ON UPDATE CASCADE;
